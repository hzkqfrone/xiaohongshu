    ...
    <div>
        <Row :gutter="10">
            ...
            <Card>
                ...
                <div slot="extra">
                    <Button icon="md-undo" @click="cancelBtn">取消</Button>
                </div>
                ...
                <div class="publish">  
                    <Form :label-width="$store.state.app.isFromMobile?auto:170" :label-position="$store.state.app.isFromMobile?'top':'right'" class="addContent" ref="params" :model="params" :rules="rules" >
                        ...
                        <!-- 选择完用户后 显示对应的博主 -->
                        <Form-item label="选择博主" v-show="params.member_id && !isAuth[2]">  
                    ...
                    <div class="footerBtn">
                        <Button size="large" icon="md-undo" @click="cancelBtn">取消</Button>
                        <Button size="large" type="success" :loading="btnLoading" icon="ios-folder-open" @click="add_manuscript(0)">保存</Button>         
                        <Button size="large" type="primary" :loading="btnLoading" icon="md-checkmark" @click="add_manuscript(1)">发布</Button>           
                    </div>
                </div>
            </Card>
            </Col>
...
<script>
    import { beyondSize, userAuthority, uploadApiEncrypt, ImgEmoji, getUserList } from '@/mixins/mixin';
    import { getScreenArticleDetails, screenAddArticle, getUserInBloggerList } from '@/libs/api';
    import editor from '@/views/my-components/editor/editor'; 
    export default {
        mixins:[beyondSize, userAuthority, uploadApiEncrypt, ImgEmoji, getUserList],
        data(){
            return {
                btnLoading: false,         //发布按钮 loading
                rules: {
                    brandTitle: [
        ...
        methods:{
            ...
            //发布 || 保存
            pub(){
                screenAddArticle(this.params).then(res => {

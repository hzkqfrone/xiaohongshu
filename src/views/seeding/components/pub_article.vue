...
<script>
    ...
    import { seedingArticelPub, seedingArticlePubDetail } from '@/libs/api';
    ...
    export default {
        ...
        methods:{
            ...
            //发布 || 保存
            pub(){
                this.params.ids = this.editId;
                seedingArticelPub(this.params).then(res => {
            ...
            //获取编辑详情
            getDetail(){
                seedingArticlePubDetail({ids: this.editId}).then(res => {
                    if(res.code == 200){
                        this.$nextTick(() => {
                            ...
                            setTimeout(() => {
                                this.$refs.textarea.forEach((v, index) => {
                                    if(v.$refs.ueditor.editor.body.innerHTML){
                                        v.$refs.ueditor.editor.body.innerHTML =  res.data.content[index]
                                    }
                                })
                            }, 600);
